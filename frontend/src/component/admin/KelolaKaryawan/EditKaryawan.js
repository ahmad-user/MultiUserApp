...
const EditKaryawan = () => {
  const { id } = useParams(); // Ambil ID karyawan dari parameter route
  const [karyawan, setKaryawan] = useState({
    ...
    Jenkel: "",
    Agama: "",
    Gol: "", // Sesuaikan dengan nama yang digunakan di form
    Status: "",
    AlamatKtp: "",
    AlamatTinggal: "",
    Email: "",
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(`http://localhost:5000/karyawan/${id}`, karyawan, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
